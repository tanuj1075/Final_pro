...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!is_valid_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        $saveResult = $db->upsertAnimeContent($_POST);
        if ($saveResult === true) {
            $animeId = (int)($_POST['id'] ?? 0);
            if ($animeId === 0) {
                $latest = $db->searchAnime(trim((string)($_POST['title'] ?? '')), '', '', 'newest');
            ...
            if ($animeId > 0 && $episodeNumber > 0 && $releaseDate !== '') {
                $db->addOrUpdateReleaseSchedule($animeId, $episodeNumber, $releaseDate, $scheduleStatus);
            }
            $flash = 'Anime content saved successfully.';
        } else {
            $flash = $saveResult;
        }
    }
    ...
    <div class="card-body">
      ...
      <form method="post">
        ...
        <div class="form-row">
          ...
          <div class="col-md-4 mb-2"><input class="form-control" name="title" required placeholder="Title"></div>
          <div class="col-md-2 mb-2"><input class="form-control" name="rating" type="number" min="0" max="10" step="0.1" placeholder="Rating"></div>
          ...
          <div class="col-md-2 mb-2"><input class="form-control" name="total_episodes" type="number" min="0" placeholder="Episodes"></div>
        </div>
  ...
  <div class="card"><div class="card-body">
    ...
    <div class="table-responsive">
      <table class="table table-sm table-striped">
        <thead><tr><th>ID</th><th>Title</th><th>Status</th><th>Rating</th><th>Episodes</th></tr></thead>
        <tbody>
